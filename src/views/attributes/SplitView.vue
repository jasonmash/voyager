        ...
        <div class="h-100">
          ...
          <b-list-group flush class="list-content" @keydown.up.down="onKeyUp">
            <div class="border-bottom pt-1">
              <b-list-group-item :ref="`i-${index}`" v-for="(item, index) in list" :key="index" @click="setSelectedIndex(index)" v-bind:to="'/attributes/' + item.key" :replace="!!$route.params.pathMatch" class="px-3 py-2">
                <p class="mb-0">
                  {{item.friendlyName}}
                  <span class="float-right" v-show="$route.params.pathMatch == item.key"><i class="fa fa-chevron-right"></i></span>
                </p>
              </b-list-group-item>
            </div>
          </b-list-group>
